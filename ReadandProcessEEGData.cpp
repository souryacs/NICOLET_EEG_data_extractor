...
void CEEG_marking_toolDlg::Read_and_Process_EEG_Data(bool binary_data_read)
{
	int ch_no;
	FILE *fid;
	int noi, nof;
...
#ifdef PATTERN_DETECTION
  if (binary_data_read == false)
	{
		...
		if ((fileName.Find(".txt") != -1) && (!(CFile::GetStatus(outfile, status))))	
		{
			...
			//svm_params.degree = 3;
			//svm_params.coef0 = 1;
			svm_params.gamma = (0.2 * 0.000001);	//0.000031
			//svm_params.nu = 0.5;
			svm_params.C = 100.0;		// 512.0
			...
			svm_params.gamma = (3.0 * 0.000001);	//0.000122;
			svm_params.C = 1.0;	//16.0;
