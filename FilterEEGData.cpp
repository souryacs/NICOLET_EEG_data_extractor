...
void CEEG_marking_toolDlg::Filter_Amp_EEG_Data()
{
	...
	double beta;
	double ripple = 0.0001;		//0.001;		// ripple
	double transWidth = 0.1;	//0.5;			// pass band step
	...
	int nx = (int)sampling_rate * TIME_LENGTH;	// data
	...
	for (ch_no = 0; ch_no < no_of_channels; ch_no++)
	{
		...
		for (time_elapsed = 0; time_elapsed < total_recording_duration_sec; (time_elapsed = time_elapsed + TIME_LENGTH))	
		{
			start_index = (int)(time_elapsed * sampling_rate);
			end_index = (int)((time_elapsed + TIME_LENGTH) * sampling_rate - 1);	
			if (end_index >= channel_data_size)
			{
				end_index = channel_data_size - 1;
			}

			// determine the signal length for both the input signal as well as the filtered output
			nx = end_index - start_index + 1;
			ny = nx + kaiserWindowLength - 1;

			// reset this variable before calling the function as then the function will workon the
			// recorded raw data
			filtering_on = false;
			// read current signal segment
			Read_Signal_Segment(ch_no, start_index, end_index, x, nx);
			// now again reset the boolean flag back to the original condition
			filtering_on = true;
